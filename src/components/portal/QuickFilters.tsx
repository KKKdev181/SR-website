  ...
  return (
    <button
      ...
      className={`inline-flex min-h-9 max-w-full items-center gap-1 rounded-full border px-3.5 py-1 text-xs font-medium transition-all duration-150 ${
        isActive
          ? "border-cyan-200 bg-cyan-100 text-[#0f1931] shadow-sm"
          : "border-cyan-200/20 bg-[#15233e]/70 text-slate-100 shadow-sm hover:border-cyan-200/45 hover:bg-[#1b2c4d] hover:text-white"
      }`}
    ...
    >
      ...
      {arLabel && (
        <>
          <span className={isActive ? "text-[#0f1931]/35" : "text-cyan-100/35"}>|</span>
...
const QuickFilters = ({ activeFilters, onToggleFilter }: QuickFiltersProps) => {
  ...
  return (
    <div className="border-b border-cyan-200/10 bg-[#0f1931]/95 backdrop-blur">
      <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6">
        <div className="rounded-2xl border border-cyan-200/20 bg-[#15233e]/80 p-3 shadow-xl shadow-black/15 backdrop-blur">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="flex min-w-fit items-center gap-2 text-xs">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-100 text-primary shadow-sm">
            ...
            <div className="relative flex shrink-0 gap-2">
              <button
                ...
                className="inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-cyan-200/20 bg-[#0f1931]/80 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition-colors hover:border-cyan-200/45 hover:bg-[#1b2c4d]"
          ...
          {showMore && (
            <div className="mt-3 rounded-xl border border-cyan-200/15 bg-[#0b1326]/80 p-3">
