...
const Header = ({ searchQuery, onSearchChange }: HeaderProps) => {
  return (
    <header className="sticky top-0 z-50 border-b border-primary/10 bg-white/92 shadow-sm backdrop-blur-xl">
      <div className="bg-primary">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-4">
            <div className="flex h-12 w-24 shrink-0 items-center justify-center rounded-xl bg-white/10 px-3">
              <img src={elmLogo} alt="Elm Logo" className="h-9 w-auto brightness-0 invert" />
            </div>

            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                <h1 className="text-base font-semibold leading-tight text-primary-foreground sm:text-lg">
                ...
                <span
                  dir="rtl"
                  lang="ar"
                  className="text-base font-semibold leading-tight text-primary-foreground/90 sm:text-lg"
                  style={arabicFont}
                >
                  مركز التقنية
                </span>
              </div>
              <p className="mt-1 max-w-2xl text-xs leading-relaxed text-primary-foreground/72">
                Find the right technology request, prepare what is needed, then open it in Jira.
          ...
          <div className="flex flex-wrap items-center gap-2.5 lg:justify-end">
            ...
            <div className="group relative">
              ...
              <div className="pointer-events-none absolute right-0 top-12 z-[60] w-[min(24rem,calc(100vw-2rem))] rounded-xl border border-border bg-background p-4 text-left opacity-0 shadow-xl transition-opacity duration-150 group-hover:opacity-100">
                <p className="text-sm font-semibold text-foreground">About Technology Center Portal</p>
      ...
      <div className="bg-gradient-to-b from-white to-slate-50">
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6">
          <div className="relative mx-auto w-full max-w-4xl">
            <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-primary/65" />
            <Input
              value={searchQuery}
              onChange={(event) => onSearchChange(event.target.value)}
              placeholder="Search by request, keyword, environment... | ابحث باسم الطلب أو البيئة أو الكلمة المفتاحية..."
              className="h-12 rounded-xl border-slate-200 bg-white pl-11 pr-4 text-sm shadow-sm transition-shadow focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20"
            />
          </div>
        </div>
      </div>
