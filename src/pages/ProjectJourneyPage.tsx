...
const ProjectJourneyPage = () => {
  ...
  useEffect(() => {
    ...
    const apply = () => localizeProjectJourney(root, isArabic);
    apply();
      ...
      <main className="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <section className="journey-page-hero">
          ...
          <div>
            ...
            <p>
              {isArabic
                ? "راجع رحلة المشروع كاملة من التحضير وحتى الإطلاق. افتح كل مرحلة للاطلاع على الطلبات والمتطلبات والملاحظات وروابط Jira المرتبطة بها."
                : "Review the complete project journey from preparation to go-live. Open each stage to see its requests, requirements, notes, and related Jira links."}
            </p>
          </div>
        </section>
        ...
        <section className="journey-page-overview" aria-label={isArabic ? "مراحل رحلة المشروع" : "Project journey stages"}>
          <div className="journey-page-overview-copy">
            <span>{isArabic ? "مسار واضح خطوة بخطوة" : "A clear step-by-step path"}</span>
            <h2>{isArabic ? "اتبع المراحل بالترتيب" : "Follow the journey in order"}</h2>
            <p>
              {isArabic
                ? "استخدم البطاقات التالية للانتقال مباشرة إلى المرحلة المطلوبة."
                : "Use the stage cards below to jump directly to the part you need."}
            </p>
          </div>
